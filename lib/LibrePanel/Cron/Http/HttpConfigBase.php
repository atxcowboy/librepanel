...
EOC;
    ...
    public function init()
    {
        ...
        if (Settings::Get('system.leenabled') == 1) {
            LibrePanelLogger::getInstanceOf()->logAction(
                ...
                LOG_INFO,
                'Running Let\'s Encrypt cronjob prior to regenerating webserver config files'
            );
    ...
    public function reload()
    {
            ...
            foreach ($restart_cmds as $restart_cmd) {
                ...
                if ($_conffiles === false || empty($_conffiles)) {
                    LibrePanelLogger::getInstanceOf()->logAction(
                        LibrePanelLogger::CRON_ACTION,
                        LOG_INFO,
                        $called_class . '::reload: fpm config directory "' . $restart_cmd['config_dir'] . '" is empty. Creating dummy.'
                    );
                ...
                LibrePanelLogger::getInstanceOf()->logAction(
                    LibrePanelLogger::CRON_ACTION,
                    LOG_INFO,
                    $called_class . '::reload: running ' . $restart_cmd['reload_cmd']
                );
                FileDir::safe_exec(escapeshellcmd($restart_cmd['reload_cmd']));
            }
        }

        // Determine which reload command to use based on the webserver
        if (Settings::Get('system.webserver') == "caddy") {
            $reload_cmd = Settings::Get('system.caddyreload_command');
        } elseif (Settings::Get('system.webserver') == "apache2") {
            $reload_cmd = Settings::Get('system.apachereload_command');
        } else {
            // Fallback (e.g. for nginx)
            $reload_cmd = Settings::Get('system.apachereload_command');
        }
        ...
        LibrePanelLogger::getInstanceOf()->logAction(
            LibrePanelLogger::CRON_ACTION,
            LOG_INFO,
            $called_class . '::reload: reloading ' . $called_class
        );
        FileDir::safe_exec(escapeshellcmd($reload_cmd));

        /**
         * nginx and caddy do not auto-spawn fcgi-processes
         */
        if ((Settings::Get('system.webserver') == "nginx" || Settings::Get('system.webserver') == "caddy")
            && Settings::Get('system.phpreload_command') != ''
            && (int)Settings::Get('phpfpm.enabled') == 0
        ) {
            LibrePanelLogger::getInstanceOf()->logAction(
                LibrePanelLogger::CRON_ACTION,
                LOG_INFO,
                $called_class . '::reload: restarting php processes'
            );
    ...
    protected function getVhostFilename(array $domain, bool $ssl_vhost = false, bool $filename_only = false)
    {
        ...
        if (Settings::Get('system.webserver') == "caddy") {
            $filename = $vhost_no . '_librepanel_' . ($ssl_vhost ? 'ssl' : 'normal') . '_vhost_' . $domain['domain'] . '.caddy.conf';
            if ($filename_only) {
                return $filename;
            }
            // use caddy configuration directory if set, otherwise fall back to apacheconf_vhost
            $caddy_conf = Settings::Get('system.caddyconf') ?: Settings::Get('system.apacheconf_vhost');
            return FileDir::makeCorrectFile($caddy_conf . '/' . $filename);
        } else {
    ...
    {
        if (Settings::Get('system.webserver') == "caddy") {
            $vhosts_folder = FileDir::makeCorrectDir(Settings::Get('system.caddyconf') ?: dirname(Settings::Get('system.apacheconf_vhost')));
        } else {
