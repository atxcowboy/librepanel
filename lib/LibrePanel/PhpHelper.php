...
class PhpHelper
{
	...
	public static function phpErrHandler($errno, $errstr, $errfile, $errline)
	{
		...
		if (!isset($_SERVER['SHELL']) || (isset($_SERVER['SHELL']) && $_SERVER['SHELL'] == '')) {
			...
			$errfile = str_replace(LibrePanel::getInstallDir(), "", $errfile);
			// build alert
			$type = 'danger';
			...
			foreach ($debug as $dline) {
				$err_display .= $dline['function'] . '() called at [' . str_replace(LibrePanel::getInstallDir(), '',
	...
	{
		$str = sprintf("<?php\n// %s\n\n", $comment ?? 'autogenerated librepanel file');
